...
def main(**kwargs):
    ...
    parser.add_argument('--loss', type=str,default='mae',help='mae, mse, mae-focal, mse-focal, bmse1, bmse9, huber, kirtosis, Gumbel')
    ...
    save_path       = f"archive/{args.loss}/{config['start_up']['model_name']}_{dataset_name}.pt"             # the best model
    save_path_resume= f"archive/{args.loss}/{config['start_up']['model_name']}_{dataset_name}_resume.pt"      # the resume model
...
# ============================= Model and Trainer ============================= #
    ...
    if args.loss == 'mse':
        ...
        loss = masked_focal_mae_loss
    elif args.loss == 'mse-focal':
        loss = masked_focal_mse_loss
    elif args.loss == 'bmse1':
        loss = masked_bmc_loss_1
    elif args.loss == 'bmse9':
        loss = masked_bmc_loss_9
    elif args.loss == 'huber':
        loss = masked_huber
    elif args.loss == 'kirtosis':
        loss = masked_kirtosis
    elif args.loss == 'Gumbel':
        loss = masked_Gumbel
    else:
        loss = masked_mae
    # get a trainer
    engine  = trainer(scaler, model, loss, **optim_args)
    ...
    if (mode == 'resume') and engine.end_of_cl():
        mvalid_loss, mvalid_mae, mvalid_mape, mvalid_rmse = engine.eval(device, dataloader, model_name, _max=_max, _min=_min)
    ...
    if mode != 'test':
        for epoch in range(resume_epoch + 1, optim_args['epochs']):
            ...
            for itera, (x, y) in enumerate(dataloader['train_loader'].get_iterator()):
                ...
                loss, mae, mape, rmse = engine.train(trainx, trainy, batch_num=batch_num, _max=_max, _min=_min)
                #print(f"{itera}: Loss: {loss}, MAE: {mae}".format(itera, loss), end='\r')
...
# =============================================================== Validation ================================================================= #
            ...
            log = 'Current Time: ' + curr_time + ' | Epoch: {:03d} | Train_Loss: {:.4f} | Train_MAE: {:.4f} | Train_MAPE: {:.4f} | Train_RMSE: {:.4f} | Valid_Loss: {:.4f} | Valid_MAE: {:.4f} | Valid_RMSE: {:.4f} | Valid_MAPE: {:.4f} | LR: {:.6f}'
            print(log.format(epoch, mtrain_loss, mtrain_mae, mtrain_mape, mtrain_rmse, mvalid_loss, mvalid_mae, mvalid_rmse, mvalid_mape, current_learning_rate))
            if engine.end_of_cl():
                early_stopping(mvalid_loss, engine.model)
            if early_stopping.early_stop:
                ...
                break
# =============================================================== Test ================================================================= #
            engine.test(model, save_path_resume, device, dataloader, scaler, args.loss, _max=_max, _min=_min, dataset_name=dataset_name)
            print(f"Model Saved at Epoch {epoch}.")

        print("Average Training Time: {:.4f} secs/epoch".format(np.mean(train_time)))
        print("Average Inference Time: {:.4f} secs/epoch".format(np.mean(val_time)))

        engine.test(model, save_path_resume, device, dataloader, scaler, args.loss, save=False, _max=_max, _min=_min, dataset_name=dataset_name)
    else:
        engine.test(model, save_path_resume, device, dataloader, scaler, args.loss, save=False, _max=_max, _min=_min, dataset_name=dataset_name)
